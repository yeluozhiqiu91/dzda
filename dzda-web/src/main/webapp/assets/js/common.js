...
function _gotoBtn() {
    $('#gotoBtn').on('click', function() {
        ...
        var skip = $('#p_pageIndex').val();
        var questionPost = url.indexOf('?') > -1;
        url = url.replace(/page=[0-9]*\&*|\&*page=[0-9]*/gi, '');
        window.location.href = url + (questionPost ? '&' : '?') + 'page=' + skip;
    })
    $('#p_pageIndex').on('keyup', function(e) {
        e.keyCode === 13 && $('#gotoBtn').trigger('click')
    })
}