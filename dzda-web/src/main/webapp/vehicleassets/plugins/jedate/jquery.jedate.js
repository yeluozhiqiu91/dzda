...
;(function(root, factory) {
    ...
    jedfn.generateHtml = function(opts,boxCell){
        ...
        var hmsStrHtml = '<div class="jedatehmscon jedateprophours"></div><div class="jedatehmscon jedatepropminutes"></div><div class="jedatehmscon jedatepropseconds"></div>';

        var botStrHtml = '<div class="botflex jedatehmsshde"><ul class="jedatehms"><li><input type="text" maxlength="2" numval="23" hms="0"><i>:</i></li><li><input type="text" maxlength="2" numval="59" hms="1"><i>:</i></li><li style="padding:0;"><input type="text" maxlength="2" numval="59" hms="2"></li></ul></div>' +
    ...
    jedfn.eventsDate = function(opts,boxCell) {
        ...
        if(/\hh/.test(jet.format)){
            var hsCls = boxCell.find(".jedateprophours"),
                ...
                hmsStr = that.eachStrhms(opts, boxCell);

            hsCls.html(hmsStr[0]);
            msCls.html(hmsStr[1]);
            ssCls.html(hmsStr[2]);

            prophms.on('click', 'p', function() {
                var _this = $(this);
                if (_this.hasClass("disabled")) return;
                // _this.addClass('action').siblings().removeClass('action');
                prophms.find('.jedatehmscon').hide();
                boxCell.find(".jedatebot .jedatehms input").eq(idx).val(jet.digit(_this.text()));
            })

            boxCell.on('click', '.jedatehms input', function() {
                idx = $(this).parent().index();
                prophms.show().find('.jedatehmscon').eq(idx).show().siblings().hide();
            })
        ...
        boxCell.find(".jedatebot .jedateok").on("click", function(ev) {
            ...
            that.dateClose();
            if ($.isFunction(opts.choosefun) || opts.choosefun != null) opts.choosefun(elemCell,okVal,okdate);
    ...
    jedfn.chooseDays = function(opts,boxCell) {
        ...
        boxCell.find(".jedaul li").on("click", function(ev) {
            ...
            ev.stopPropagation();
            if(/\hh/.test(jet.format)){
                boxCell.children("ul").attr("dateval", _that.attr("data-ymd"));
                _that.addClass('action').siblings().removeClass('action');
                return;
            }
            var dateArr = jet.reMacth(_that.attr("data-ymd")),
                getDateVal = jet.parse([ dateArr[0], dateArr[1], dateArr[2] ], [], thisformat);
            ...
            if ($.isFunction(opts.choosefun) || opts.choosefun != null){
                opts.choosefun && opts.choosefun(objCell,getDateVal);
            }
        });
